...
describe('Expression Evaluation', () => {
  ...
  describe('Conversion Function Robustness', () => {
      ...
      it('should convert boolean to string representation', () => {
          const exprTrue = new Expression(undefined, [createFunctionEl('string', new Expression(undefined, [createLiteral(true)]))]);
          expect(evalExpr(exprTrue.rightSide)).toBe('true');
          const exprFalse = new Expression(undefined, [createFunctionEl('string', new Expression(undefined, [createLiteral(false)]))]);
          expect(evalExpr(exprFalse.rightSide)).toBe('false');
      });

      it('should handle chained function calls', () => {
          const inner = new Expression(undefined, [createLiteral('123')]);
          const mid = new Expression(undefined, [createFunctionEl('float', inner)]);
          const outer = new Expression(undefined, [createFunctionEl('integer', mid)]);
          expect(evalExpr(outer.rightSide)).toBe(123);
      });

      it('should validate function return types', () => {
          const expr = new Expression(undefined, [
              createFunctionEl('integer', new Expression(undefined, [createLiteral("'abc'")]))
          ]);
          expect(() => evalExpr(expr.rightSide)).toThrow();
      });

      it('should throw for undefined conversion function', () => {
          const expr = new Expression(undefined, [createFunctionEl('undefinedFunc', new Expression(undefined, []))]);
          expect(() => evalExpr(expr.rightSide)).toThrow();
      });

      it('should handle empty function arguments', () => {
          const expr = new Expression(undefined, [createFunctionEl('float', new Expression(undefined, []))]);
          expect(() => evalExpr(expr.rightSide)).toThrow();
      });

      it('should convert float variable to string representation', () => {
          ...
          expect(expr.calculateValue([funcEl], 'string', valuedVariables)).toBe('3.5');
      });
  });
  ...
  describe('Arithmetic and Edge Cases', () => {
      ...
      it('should handle modulo with negative numbers', () => {
          expect(evalExpr([createLiteral(-5), createOperator('%'), createLiteral(3)])).toBe(-2);
          expect(evalExpr([createLiteral(5), createOperator('%'), createLiteral(-3)])).toBe(2);
      });

      it('should throw for expression with only operator', () => {
          const expr = new Expression(undefined, [createOperator('+')]);
          expect(() => evalExpr(expr.rightSide)).toThrow();
      });

      it('should throw for mismatched parentheses', () => {
          const elements = [
              createOperator('('), createLiteral(5), createOperator('+'), createLiteral(3)
          ];
          expect(() => evalExpr(elements)).toThrow();
      });

      it('should handle integer overflow', () => {
          const maxInt = 2**31 - 1;
          const elements = [createLiteral(maxInt), createOperator('+'), createLiteral(1)];
          // Note: JavaScript doesn't throw on integer overflow, it just returns a larger number
          // This test documents the current behavior
          expect(evalExpr(elements, 'integer')).toBe(maxInt + 1);
      });
  });
  ...
  describe('Relational Operators', () => {
      ...
      it('should throw when comparing string with number', () => {
          const expr = new Expression([createLiteral("'10'")], [createLiteral(10)], '==');
          expect(() => expr.evaluate([])).toThrow();
      });

      it('should implicitly convert int to float in comparisons', () => {
  ...
  describe('Logical Operators', () => {
      ...
      it('should handle nested logical operators', () => {
          const elements = [
              createOperator('('),
              createLiteral(true), createOperator('&&'), createLiteral(false),
              createOperator(')'),
              createOperator('||'),
              createLiteral(true)
          ];
          expect(evalExpr(elements)).toBe(true);
      });
  });
  ...
  describe('Assignment and Context', () => {
      ...
      it('should handle variables with whitespace in names', () => {
          const varSpace = new Variable('var-space', 'string', 'full name', 'node-1');
          const valuedSpace = new ValuedVariable('var-space', 'string', 'full name', 'node-1', 'John Doe');
          const expr = new Expression(undefined, [createVariableEl(varSpace)]);
          expect(evalExpr(expr.rightSide, 'string', [valuedSpace])).toBe('John Doe');
      });
  });
});
...
describe('Disallowed Concatenations', () => {
    it('should throw when concatenating number + string (order swapped)', () => {
        const elements = [createLiteral(10), createOperator('+'), createLiteral("'Score: '")];
        const expr = new Expression(undefined, elements);
        expect(() => expr.calculateValue(elements, null, []))
            .toThrow('Cannot apply operator "+" to types integer and string.');
    });

    it('should throw when concatenating string + boolean', () => {
        const elements = [createLiteral("'Status: '"), createOperator('+'), createLiteral(true)];
        const expr = new Expression(undefined, elements);
        expect(() => expr.calculateValue(elements, null, []))
            .toThrow('Cannot apply operator "+" to types string and boolean.');
    });

    it('should throw when concatenating boolean + string', () => {
        const elements = [createLiteral(true), createOperator('+'), createLiteral("'Status: '")];
        const expr = new Expression(undefined, elements);
        expect(() => expr.calculateValue(elements, null, []))
            .toThrow('Cannot apply operator "+" to types boolean and string.');
    });

    it('should throw when concatenating string + float', () => {
        const elements = [createLiteral("'Value: '"), createOperator('+'), createLiteral(3.14)];
        const expr = new Expression(undefined, elements);
        expect(() => expr.calculateValue(elements, null, []))
            .toThrow('Cannot apply operator "+" to types string and float.');
    });

    it('should throw when concatenating float + string', () => {
        const elements = [createLiteral(3.14), createOperator('+'), createLiteral("'Value: '")];
        const expr = new Expression(undefined, elements);
        expect(() => expr.calculateValue(elements, null, []))
            .toThrow('Cannot apply operator "+" to types float and string.');
    });
});

describe('Disallowed Comparisons', () => {
    it('should throw when comparing string vs boolean', () => {
        const expr = new Expression([createLiteral("'true'")], [createLiteral(true)], '==');
        expect(() => expr.evaluate([]))
            .toThrow('Cannot compare values of different types: string and boolean.');
    });
});

describe('Assignment Type Mismatches', () => {
    it('should throw when assigning a string to an integer variable', () => {
        const expr = new Expression(varIntX, [createLiteral("'text'")]);
        expect(() => expr.assignValue(valuedVariables))
            .toThrow("Type mismatch: Cannot assign a value of type 'string' to a variable of type 'integer'.");
    });

    it('should throw when assigning a string to a float variable', () => {
        const expr = new Expression(varFloatA, [createLiteral("'text'")]);
        expect(() => expr.assignValue(valuedVariables))
            .toThrow("Type mismatch: Cannot assign a value of type 'string' to a variable of type 'float'.");
    });

    it('should throw when assigning string literal "true" to a boolean variable', () => {
        const expr = new Expression(varBoolB, [createLiteral("'true'")]);
        expect(() => expr.assignValue(valuedVariables))
            .toThrow("Type mismatch: Cannot assign a value of type 'string' to a variable of type 'boolean'.");
    });

    it('should throw when assigning a boolean to an integer variable', () => {
        const expr = new Expression(varIntX, [createLiteral(true)]);
        expect(() => expr.assignValue(valuedVariables))
            .toThrow("Type mismatch: Cannot assign a value of type 'boolean' to a variable of type 'integer'.");
    });

    it('should throw when assigning a boolean to a float variable', () => {
        const expr = new Expression(varFloatA, [createLiteral(false)]);
        expect(() => expr.assignValue(valuedVariables))
            .toThrow("Type mismatch: Cannot assign a value of type 'boolean' to a variable of type 'float'.");
    });
});
});